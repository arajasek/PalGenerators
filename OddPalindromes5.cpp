...
//Q States are 6-tuples (carry, x,y, k, l1,l2)
// a carry
...
private:
	string name;
	int maxCarry;
	string getQStateName(int carry, int x, int y, int k, int l1, int l2);
	void createStates();
...
string AutomatonGenerator::getQStateName(int carry, int x, int y, int k, int l1, int l2) {
	return ("q_"+to_string(carry)+"_"+to_string(x)+to_string(y)+"_"
		+to_string(k)+"_"+to_string(l1)+to_string(l2));
}
					...
					for(int k =0; k <= 1; k++) {
						for (int l1 = 0; l1 <= 1; l1++) {
							for (int l2 = 0; l2 <= 1; l2++) {
								string topName = getQStateName(carry, x, y, k, l1, l2);
								int bit = (stateCarry + k + l2) % 2;
								int newCarry = (stateCarry + k + l2) / 2;
								char inp = 'e' + bit;
	...
	for (int carry = 0; carry <= maxCarry; carry++) {
		for (int x = 0; x <= 1; x++) {
			for (int y = 0; y <= 1; y++) {
				for(int k =0; k <= 1; k++) {
					for (int l1 = 0; l1 <= 1; l1++) {
						string name = getQStateName(carry, x, y, k, l1, l1);
...
void AutomatonGenerator::printInternalTransitions() {
	...
	for (int carry = 0; carry <= maxCarry; carry++) {
		for (int x = 0; x <= 1; x++) {
			for (int y = 0; y <= 1; y++) {
				for (int l1 = 0; l1 <= 1; l1++) {
					string name = getQStateName(carry, x, y, x, l1, y);
					int bit = (carry + x + y) % 2;
		...
		for (int x = 0; x <= 1; x++) {
			for (int y = 0; y <= 1; y++) {
				for(int k =0; k <= 1; k++) {
					for (int l1 = 0; l1 <= 1; l1++) {
						for (int l2 = 0; l2 <= 1; l2++) {
							string name = getQStateName(carry, x, y, k, l1, l2);
							int bit = (carry + x + y) % 2;
							int newCarry = (carry + x + y) / 2;
							char inp = 'a' + bit;
							for (int x2 = 0; x2 <= 1; x2++) {
								for (int y2 = 0; y2 <= 1; y2++) {
										cout<<"("<<name<<" "<<inp<<" "<<getQStateName(newCarry, x2, y2, k, y, l1)<<")\n";
								}
							}
						}
					}
		...
		for (int x = 0; x <= 1; x++) {
			for (int y = 0; y <= 1; y++) {
				for(int k =0; k <= 1; k++) {
					for (int l1 = 0; l1 <= 1; l1++) {
						for (int l2 = 0; l2 <= 1; l2++) {
							cout<<getQStateName(carry, x, y, k, l1, l2)<<endl;
						}
					}
				}
			}
