...
//Q States are 7-tuples (carry, x,z, k, m1, m2, m3)
// a carry
...
private:
	string name;
	int maxCarry;
	string getQStateName(int carry, int x, int z, int k, int m1, int m2, int m3);
	void createStates();
...
string AutomatonGenerator::getQStateName(int carry, int x, int z, int k, int m1, int m2, int m3) {
						...
						for (int m1 = 0; m1 <= 1; m1++) {
							for (int m2 = 0; m2 <= 1; m2++) {
								for (int m3 = 0; m3 <= 1; m3++) {
									string topName = getQStateName(carry, x, z, k, m1, m2, m3);
									int bit = (stateCarry + k + m3) % 2;
									int newCarry = (stateCarry + k + m3) / 2;
									char inp = 'e' + bit;
		...
		for (int x = 0; x <= 1; x++) {
			for (int z = 0; z <= 1; z++) {
				for(int k =0; k <= 1; k++) {
					for (int m1 = 0; m1 <= 1; m1++) {
						for (int m2 = 0; m2 <= 1; m2++) {
							string name = getQStateName(carry, x, z, k, m1, m2, m1);
...
void AutomatonGenerator::printInternalTransitions() {
	...
	for (int carry = 0; carry <= maxCarry; carry++) {
		for (int x = 0; x <= 1; x++) {
			for (int z = 0; z <= 1; z++) {
				for (int m1 = 0; m1 <= 1; m1++) {
					string name = getQStateName(carry, x, z, x, m1, m1, z);
					int bit = (carry + x + z) % 2;
			...
			for (int z = 0; z <= 1; z++) {
				for(int k =0; k <= 1; k++) {
					for (int m1 = 0; m1 <= 1; m1++) {
						for (int m2 = 0; m2 <= 1; m2++) {
							for (int m3 = 0; m3 <= 1; m3++) {
								string name = getQStateName(carry, x, z, k, m1, m2, m3);
								int bit = (carry + x + z) % 2;
								int newCarry = (carry + x +z) / 2;
								char inp = 'a' + bit;
								for (int x2 = 0; x2 <= 1; x2++) {
									for (int z2 = 0; z2 <= 1; z2++) {
										cout<<"("<<name<<" "<<inp<<" "<<getQStateName(newCarry, x2, z2, k, z, m1, m2)<<")\n";
									}
								}
							}
						}
			...
			for (int z = 0; z <= 1; z++) {
				for(int k =0; k <= 1; k++) {
					for (int m1 = 0; m1 <= 1; m1++) {
						for (int m2 = 0; m2 <= 1; m2++) {
							for (int m3 = 0; m3 <= 1; m3++) {
								cout<<getQStateName(carry, x, z, k, m1, m2, m3)<<endl;
							}
						}
					}
				}
